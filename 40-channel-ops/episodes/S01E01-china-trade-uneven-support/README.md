...
本集的选题、研报主线、V5 脚本方向、标题和封皮文案已经锁定；封皮视觉规范已形成第一版，进入真实 PDF 首页执行验证阶段。
...
当前推荐视觉：真实 J.P. Morgan 第 1 页 + Editorial Split 版式 + 暖白底 + 墨黑文字 + 深酒红强调 `25%`，不加人物、港口、集装箱、国旗、K 线、其他频道标识或 AI 重绘研报。
...
1. 按 `61-thumbnail-spec-v1.md`，基于 **真实 PDF 第 1 页** 做第一张可评审封皮；
2. 封皮先验证真实感、阅读层级、缩略图可读性和高级感，不追求一次定稿；
3. 封皮方向基本通过后，再用 `40-script-v5.md` 生成第一版 TTS；
4. TTS 只做听感校验：句子是否拗口、术语是否难听懂、段落是否拖、总时长是否合适；
5. 音频锁定后生成 SRT，再进入视觉导演 / Storyboard。
